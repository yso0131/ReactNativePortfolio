...
async function validateExpoServersAsync(projectRoot) {
  const domains = ['expo.io', 'expo.fyi', 'expo.dev', 'static.expo.dev' // 'exp.host', - This is causing some intermittent false errors
  ];
  ...
  if (failures.length) {
    failures.forEach(failure => {
      ProjectUtils().logWarning(projectRoot, 'expo', `Warning: could not reach \`${failure.domain}\`.`, `doctor-server-dashboard-not-reachable-${failure.domain}`);
    });
    console.log();
    ProjectUtils().logWarning(projectRoot, 'expo', `We couldn't reach some of our domains, this might cause issues on our website or services.\nPlease check your network configuration and try to access these domains in your browser.`, 'doctor-server-dashboard-not-reachable');
    console.log();
    return WARNING;
  }
