...
async function exportAppAsync(projectRoot, publicUrl, assetUrl, outputDir, options = {}, experimentalBundle) {
  ...
  if (!experimentalBundle) {
    const validPostExportHooks = prepareHooks(hooks, 'postExport', projectRoot); // Add assetUrl to manifest
...
async function publishAsync(projectRoot, options = {}) {
  ...
  const validPostPublishHooks = prepareHooks(hooks, 'postPublish', projectRoot);
...
async function startDevServerAsync(projectRoot, startOptions) {
  (0, _errors().assertValidProjectRoot)(projectRoot);
  const port = startOptions.devClient ? Number(process.env.RCT_METRO_PORT) || 8081 : await _getFreePortAsync(19000);
  await ProjectSettings().setPackagerInfoAsync(projectRoot, {
    expoServerPort: port,
    packagerPort: port
  });
