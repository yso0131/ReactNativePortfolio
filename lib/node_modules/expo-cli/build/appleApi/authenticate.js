...
function getRequestContext(authCtx) {
  ...
  (0, _assert().assert)((_authCtx$authState = authCtx.authState) === null || _authCtx$authState === void 0 ? void 0 : _authCtx$authState.context, 'Apple request context must be defined');
...
async function loginAsync(userCredentials = {}, options) {
  ...
  (0, _assert().assert)(username); // Clear data
...
async function loginWithUserCredentialsAsync({
  ...
  (0, _assert().assert)(newSession, 'An unexpected error occurred while completing authentication'); // Success!
...
async function authenticateAsync(options = {}) {
  ...
  try {
    ...
    (0, _assert().assert)(team, `Your account is not associated with Apple Team with ID: ${teamId}`); // Get the JSON cookies in the custom YAML format used by Fastlane
