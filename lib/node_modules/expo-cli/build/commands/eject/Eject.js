...
function _configureProjectAsync() {
  const data = _interopRequireWildcard(require("../apply/configureProjectAsync"));
...
async function clearNativeFolder(projectRoot, folders) {
  ...
  try {
    await Promise.all(folders.map(folderName => _fsExtra().default.remove(_path().default.join(projectRoot, folderName))));
...
async function ejectAsync(projectRoot, {
  ...
  const results = await prebuildAsync(projectRoot, {
    platforms,
    ...options
  });
...
async function prebuildAsync(projectRoot, {
  ...
  await warnIfDependenciesRequireAdditionalSetupAsync(pkg, exp.sdkVersion, Object.keys((_managedConfig$_inter = (_managedConfig$_inter2 = managedConfig._internal) === null || _managedConfig$_inter2 === void 0 ? void 0 : _managedConfig$_inter2.pluginHistory) !== null && _managedConfig$_inter !== void 0 ? _managedConfig$_inter : {}));
  return {
    packageManager,
...
function getPackageJson(projectRoot) {
  return _jsonFile().default.read(_path().default.join(projectRoot, 'package.json'));
}
/**
...
function updatePackageJSONDependencies({
  ...
  for (const dependenciesKey of requiredDependencies) {
    ...
    if (dependenciesKey === 'react-native' && ((_pkg$dependencies = pkg.dependencies) === null || _pkg$dependencies === void 0 ? void 0 : _pkg$dependencies[dependenciesKey])) {
...
function resolveBareEntryFile(projectRoot, main) {
  ...
  if (isPkgMainExpoAppEntry(main)) return null; // Look at the `package.json`s `main` field for the main file.
...
async function warnIfDependenciesRequireAdditionalSetupAsync(pkg, sdkVersion, appliedPlugins) {
  // TODO: Remove based on plugin history
  const expoPackagesWithExtraSetup = _configureProjectAsync().expoManagedPlugins.filter(plugin => !(appliedPlugins === null || appliedPlugins === void 0 ? void 0 : appliedPlugins.includes(plugin))).reduce((prev, curr) => ({ ...prev,
    [curr]: `https://github.com/expo/expo/tree/master/packages/${curr}`
  }), {});

  const pkgsWithExtraSetup = { ...expoPackagesWithExtraSetup
  ...
  warnAdditionalSetupStep.stopAndPersist({
    symbol: '⚠️ ',
    text: _chalk().default.yellow.bold(`The app has ${packagesToWarn.length} package${plural ? 's' : ''} that require${plural ? '' : 's'} extra setup before building:`)
  });
