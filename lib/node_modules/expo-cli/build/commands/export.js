...
async function exportFilesAsync(projectRoot, options) {
  ...
  return await _xdl().Project.exportAppAsync(projectRoot, options.publicUrl, options.assetUrl, options.outputDir, exportOptions, options.experimentalBundle);
}
...
async function action(projectDir, options) {
  ...
  if (!(await CreateApp().assertFolderEmptyAsync({
    ...
    const message = `Try using a new directory name with ${_log().default.chalk.bold('--output-dir')}, moving these files, or using ${_log().default.chalk.bold('--force')} to overwrite them.`;
