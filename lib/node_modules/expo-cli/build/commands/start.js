...
function parseStartOptions(options) {
  ...
  if (options.devClient) {
    startOpts.devClient = true; // TODO: is this redundant?
...
async function startWebAction(projectDir, options) {
  ...
  if (_xdl().Versions.gteSdkVersion(exp, '34.0.0')) {
    await (0, _ensureTypeScriptSetup().ensureTypeScriptSetupAsync)(projectDir);
  }
...
async function validateDependenciesVersions(projectDir, exp, pkg) {
  ...
  const bundleNativeModulesPath = _resolveFrom().default.silent(projectDir, 'expo/bundledNativeModules.json');
