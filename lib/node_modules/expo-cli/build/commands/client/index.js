...
function _default(program) {
  program.command('client:ios [path]').helpGroup('experimental').description('Experimental: build a custom version of Expo Go for iOS using your own Apple credentials').longDescription('Build a custom version of Expo Go for iOS using your own Apple credentials and install it on your mobile device using Safari.').option('--apple-id <login>', 'Apple ID username (please also set the Apple ID password as EXPO_APPLE_PASSWORD environment variable).').asyncActionProjectDir(async (projectDir, options) => {
    ...
    const spinner = (0, _ora().default)(`Finding custom configuration for Expo Go...`).start();
    ...
    if (user && user.kind === 'user') {
      ...
      email = await (0, _prompts().promptEmailAsync)({
        ...
        initial: (_ref = context === null || context === void 0 ? void 0 : context.user) === null || _ref === void 0 ? void 0 : _ref.email
      });
    }
    ...
    if (udids.length === 0) {
      ...
      (0, _log().default)('Custom builds of Expo Go can only be installed on devices which have been registered with Apple at build-time.');
  ...
  program.command('client:install:ios').description('Install Expo Go for iOS on the simulator').option('--latest', `Install the latest version of Expo client, ignoring the current project version.`).helpGroup('client').asyncAction(async command => {
  ...
  program.command('client:install:android').description('Install Expo Go for Android on a connected device or emulator').option('--latest', `Install the latest version of Expo client, ignore the current project version.`).helpGroup('client').asyncAction(async command => {
    const forceLatest = !!command.latest;
    const currentSdkConfig = await ClientUpgradeUtils().getExpoSdkConfig(process.cwd());
    const currentSdkVersion = currentSdkConfig ? currentSdkConfig.sdkVersion : undefined;
    const sdkVersions = await _xdl().Versions.sdkVersionsAsync();
