...
const printServerInfo = async (projectDir, options = {}) => {
  ...
  const url = await _xdl().UrlUtils.constructDeepLinkAsync(projectDir);
  ...
  _log().default.nested((0, _wrapAnsi().default)(u('To run the app, choose one of:'), wrapLength)); // TODO: if dev client, chanege this message!


  _log().default.nested(item(`Scan the QR code above with the Expo app (Android) or the Camera app (iOS).`)); // TODO: if no react-native-web in package.json then don't show web info
  ...
  async function handleKeypress(key) {
    if (options.webOnly) {
      switch (key) {
        ...
        case 'e':
          (0, _log().default)(_chalk().default.red` ${BLT} Sending a URL is not supported in web-only mode`);
          break;
      }
    } else {
      switch (key) {
        ...
        case 'e':
          {
            stopWaitingForCommand();
            const lanAddress = await _xdl().UrlUtils.constructDeepLinkAsync(projectRoot, {
              hostType: 'lan'
            });
