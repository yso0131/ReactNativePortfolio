...
function resolvePackageManager(options) {
  ...
  if (options.install) {
    (0, _log().default)(packageManager === 'yarn' ? `🧶 Using Yarn to install packages. ${_chalk().default.dim('Pass --npm to use npm instead.')}` : '📦 Using npm to install packages.');
  }
...
async function installCocoaPodsAsync(projectRoot) {
  ...
  if (!(await packageManager.isCLIInstalledAsync())) {
    try {
      ...
      step.stopAndPersist({
        symbol: '⚠️ ',
        text: _log().default.chalk.red('Unable to install the CocoaPods CLI. Continuing with project sync, you can install CocoaPods CLI afterwards.')
      });

      if (e instanceof PackageManager().CocoaPodsError) {
        (0, _log().default)(e.message);
      } else {
        (0, _log().default)(`Unknown error: ${e.message}`);
      }
