...
async function updateTSConfigAsync({
  ...
  if (hasTemplateTsconfig) {
    if (projectTSConfig.extends !== _resolveModules().baseTSConfigName) {
  ...
  if (isBootstrapping) {
    (0, _log().default)(`${_chalk().default.bold`TypeScript`}: A ${_chalk().default.cyan('tsconfig.json')} has been auto-generated`);
  } else {
    (0, _log().default)(`${_chalk().default.bold`TypeScript`}: The ${_chalk().default.cyan('tsconfig.json')} has been updated ${_chalk().default.dim`(Use ${TS_FEATURE_FLAG} to skip)`}`);
...
function logModifications(modifications) {
  ...
  (0, _log().default)(`\u203A ${_chalk().default.bold('Required')} modifications made to the ${_chalk().default.cyan('tsconfig.json')}:`);
...
function printTable(items) {
  ...
  for (const [key, value] of items) {
    (0, _log().default)(tableFormat(key, value));
  }
}
